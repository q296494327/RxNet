package com.xiemiao.rxnet.interceptor;
...
public class RequestInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (logRequest) {
            ...
            //打印请求信息
            Logger.t(getTag(request, "Request_Info")).i("Params : 「 %s 」%nConnection : 「 %s 」%nHeaders : %n「 %s 」"
        ...
        if (logResponse) {
            ...
            //打印响应时间以及响应头
            Logger.t(getTag(request, "Response_Info")).i("Received response in [ %d-ms ] , [ %s ]%n%s"
    ...
    private String printResult(Request request, Response response, boolean logResponse) throws IOException {
        ...
        if (isParseable(responseBody.contentType())) {
            ...
            if (logResponse) {
                //                Logger.t(getTag(request, "Response_Result")).i(isJson(responseBody.contentType()) ?
                //                        CharacterHandler.jsonFormat(bodyString) : isXml(responseBody.contentType()) ?
                //                        CharacterHandler.xmlFormat(bodyString) : bodyString);
                Logger.t(getTag(request, "Response_Result")).i(CharacterHandler.jsonFormat(bodyString));
            }

        } else {
            if (logResponse) {
                Logger.t(getTag(request, "Response_Result")).i("This result isn't parsed");
            }
        }
        return bodyString;
    }
